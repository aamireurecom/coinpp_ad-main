...
def outer_step(
    ...
    with torch.set_grad_enabled(is_train):
        features_recon = func_rep.forward(coordinates)
        ...
        # Shape (1,)
        loss_mse = per_example_loss.mean()
        # Shape (2,) Consider both the loss stft l1 and log
        loss_stft_l1 = 0
        loss_stft_log = 0

        for d in range(dim):
            single_d_loss = loss_fn_stft.forward(features_recon[..., d], features[..., d])
            loss_stft_l1 += (single_d_loss[0] / batch_size)
            loss_stft_log += (single_d_loss[1] / batch_size)

        # print('@' * 10)
        # print(loss_stft_l1, loss_stft_log, loss_mse)

        loss = loss_mse + sftf_loss_weight * (loss_stft_l1 + loss_stft_log) / dim

    outputs = {
        "loss": loss,
        'loss_mse': loss_mse,
        'loss_stft_l1': loss_stft_l1,
        'loss_stft_log': loss_stft_log,
        "psnr": losses.mse2psnr(per_example_loss).mean(),
...
def outer_step_chunked(
    ...
    for _ in range(num_batches):
        ...
        outputs = outer_step(
            ...
            sftf_loss_weight=sftf_loss_weight,
            loss_fn_stft=loss_fn_stft,
